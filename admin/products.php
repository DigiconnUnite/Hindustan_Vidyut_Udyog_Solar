...
// Combine kits and products for display, with kind
$rows = [];
foreach ($kits as $kit) {
  $rows[] = [$kit, 'kit'];
}
...
<div class="flex flex-wrap items-center justify-between gap-3 mb-6">
  ...
  <div class="flex gap-2">
    <button onclick="openItemModal('kit')" class="btn-primary text-sm flex px-3 justify-center items-center gap-2">
      + Add Kit
    </button>
    <button onclick="openItemModal('product')" class="btn-outline text-sm flex px-3 justify-center items-center gap-2">
      + Add Component
    </button>
  </div>
</div>
...
<!-- ========================================== -->
<!-- 1. MOBILE & TABLET VIEW (Card Layout)      -->
<!-- ========================================== -->
<div class="grid grid-cols-1 gap-4 md:hidden">
  <?php foreach ($rows as [$row, $kind]): $rowIsKit = $kind === 'kit'; ?>
    <div class="card p-4 rounded-lg border border-gray-100 shadow-sm bg-white space-y-3">
      <!-- Header: Image, Name & Type -->
      <div class="flex items-start justify-between gap-3">
        <div class="flex items-center gap-3">

          <?php if ($row['image_path']): ?>
            <img src="/<?= e($row['image_path']) ?>" alt="" class="h-12 w-12 rounded object-cover flex-shrink-0">
          <?php else: ?>
            <div class="h-12 w-12 rounded bg-gray-100 flex items-center justify-center text-xs text-gray-400 flex-shrink-0">
              No Image
            </div>
          <?php endif; ?>
          ...
          <div>
            <h3 class="font-medium text-gray-900 text-sm">
              ...
              <?php if ($rowIsKit && $row['is_featured']): ?>
                <span class="badge ml-1 bg-accent-500/15 text-accent-600 inline-block text-xs">Featured</span>
              <?php endif; ?>
            </h3>
        ...
        <span class="badge flex-shrink-0 <?= $rowIsKit ? 'bg-accent-500/15 text-accent-600' : 'bg-gray-100 text-gray-600' ?>">
          <?= $rowIsKit ? 'Kit' : 'Component' ?>
        </span>
      </div>

      <!-- Content: Price, Subsidy & Status -->
      <div class="grid grid-cols-3 gap-2 py-2 border-y border-gray-100 text-xs">
        <div>
          <span class="text-gray-400 block">Price</span>
          <span class="font-medium text-gray-800"><?= $row['price'] ? '₹' . number_format((float) $row['price']) : '—' ?></span>
        </div>
        <div>
          <span class="text-gray-400 block">Subsidy</span>
          <span class="font-medium text-gray-800"><?= $rowIsKit ? '₹' . number_format((float) $row['subsidy']) : '—' ?></span>
        </div>
        <div>
      ...
      <div class="pt-2 border-t border-gray-100 flex items-center justify-end space-x-3 gap-4">
        <button onclick='openItemModal(<?= json_encode($kind) ?>, <?= json_encode($row) ?>)' class="text-primary-600 hover:underline text-xs font-medium">
          Edit
        </button>
...
<!-- ========================================== -->
<div class="card hidden md:block overflow-x-auto rounded-lg border border-gray-100 shadow-sm">
      ...
      <?php foreach ($rows as [$row, $kind]): $rowIsKit = $kind === 'kit'; ?>
        <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
          ...
          <td class="py-3 px-3 font-medium text-gray-900 whitespace-nowrap">
            <?= e($row['name']) ?>
            <?php if ($rowIsKit && $row['is_featured']): ?>
              <span class="badge ml-1 bg-accent-500/15 text-accent-600 inline-block">Featured</span>
            <?php endif; ?>
          </td>
          <td class="py-3 px-3 whitespace-nowrap">
          ...
          <td class="py-3 px-3 space-x-2 whitespace-nowrap">
...
<!-- 3. MODAL POPUP                             -->
<!-- ========================================== -->
<div id="product-modal" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center p-3 sm:p-4 z-50 overflow-y-auto">
  <div class="card w-full max-w-lg my-auto overflow-y-auto" style="max-height: 90vh;">
    ...
    <form method="post" action="/admin/products.php" enctype="multipart/form-data" class="space-y-3">
      ...
      <div>
        ...
        <input type="text" name="name" id="product-name" required class="input mt-1 w-full">
      </div>

      <!-- Kit-only fields -->
      <div class="kit-field grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label class="text-sm font-medium text-gray-700">System size (kW) *</label>
          <input type="number" name="system_kw" id="kit-kw" step="0.5" min="0.5" class="input mt-1 w-full">
        </div>
        <div>
          <label class="text-sm font-medium text-gray-700">Kit type</label>
          <select name="kit_type" id="kit-type" class="input mt-1 w-full">
      ...
      <div class="kit-field">
        ...
        <input type="text" name="suits" id="kit-suits" class="input mt-1 w-full" placeholder="3 BHK · bill ₹2,000–3,000">
      </div>
      <div class="kit-field">
        <label class="text-sm font-medium text-gray-700">Includes (one component per line)</label>
        <textarea name="includes" id="kit-includes" rows="4" class="input mt-1 w-full"></textarea>
      </div>
      <div class="kit-field grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label class="text-sm font-medium text-gray-700">Subsidy (₹)</label>
          <input type="number" name="subsidy" id="kit-subsidy" step="1" min="0" class="input mt-1 w-full" value="0">
        </div>
        <div>
          <label class="text-sm font-medium text-gray-700">Generates (units/month)</label>
          <input type="number" name="monthly_units" id="kit-units" min="0" class="input mt-1 w-full">
        </div>
      </div>
      ...
      <div class="product-field">
        ...
        <select name="category" id="product-category" class="input mt-1 w-full">
      ...
      <div class="product-field">
        ...
        <textarea name="description" id="product-description" rows="2" class="input mt-1 w-full"></textarea>
      </div>
      <div class="product-field">
        <label class="text-sm font-medium text-gray-700">Specs (pipe separated, e.g. 540W | 21% efficiency)</label>
        <input type="text" name="specs" id="product-specs" class="input mt-1 w-full">
      </div>
      <div class="product-field">
        <label class="text-sm font-medium text-gray-700">Brand</label>
        <input type="text" name="brand" id="product-brand" class="input mt-1 w-full">
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <input type="number" name="price" id="product-price" step="0.01" min="0" class="input mt-1 w-full">
        </div>
        <div class="product-field">
          <label class="text-sm font-medium text-gray-700">MRP (₹)</label>
          <input type="number" name="mrp" id="product-mrp" step="0.01" min="0" class="input mt-1 w-full">
        </div>
      </div>
      <div class="product-field grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label class="text-sm font-medium text-gray-700">Wattage (W)</label>
          <input type="number" name="wattage" id="product-wattage" min="0" class="input mt-1 w-full">
        </div>
        <div>
          <label class="text-sm font-medium text-gray-700">Warranty (years)</label>
          <input type="number" name="warranty_years" id="product-warranty" min="0" max="50" class="input mt-1 w-full">
        </div>
      </div>
      ...
      <div>
        ...
        <input type="number" name="sort_order" id="product-sort" class="input mt-1 w-full" value="0">
      </div>
      <div>
      ...
      <div class="flex gap-3 pt-2">
        <button type="button" onclick="document.getElementById('product-modal').classList.add('hidden')" class="btn-outline flex flex-1 items-center justify-center text-center">Cancel</button>
        <button type="submit" class="btn-primary flex flex-1 items-center justify-center text-center">Save</button>
      </div>
    </form>
  </div>
</div>
...
<script>
  ...
  function openItemModal(kind, row) {
    ...
    document.querySelectorAll('.kit-field').forEach(function(el) {
      el.classList.toggle('hidden', !isKit);
    });
    document.querySelectorAll('.product-field').forEach(function(el) {
      el.classList.toggle('hidden', isKit);
    });
